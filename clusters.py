...
def pearson(v1, v2):
    ...
    pSum = sum([v1[x] * v2[x] for x in range(len(v1))])
...
def kcluster(rows, distance = pearson, k = 4):
    ...
    for i in range(len(rows[0])):
        ...
        for ix in range(len(rows)):
            templist.append(rows[ix][i])
        ranges.append((min(templist),max(templist)))
...
blognames, words, data = readfile('blogdatamac.txt')
clust = hcluster(data)

printcluster(clust, blognames)


aaa = scaledown(data)
draw2d(aaa,blognames)
